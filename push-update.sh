...
updates_dir=/data/evolution_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have evolution-VERSION-DATE-TYPE-* as format"
...
# Assume evolution-VERSION-DATE-TYPE-*.zip
...
adb shell "killall org.evolution.ota 2>/dev/null"
adb shell "sqlite3 /data/data/org.evolution.ota/databases/updates.db" \
